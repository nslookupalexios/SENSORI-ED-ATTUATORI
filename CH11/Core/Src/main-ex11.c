...
int main(void) {
  ...
  while (1) {
    if (HAL_GetTick() - tick > 1000L) {
      ...
      speed = (diff*60.0)/(PULSES_PER_REVOLUTION*CPR);
      speed_sec = (diff)/(PULSES_PER_REVOLUTION*CPR);
      ...
      if ((TIM3->SMCR & 0x3) == 0x3)
        speed /= 2;
      //sto a 2
      sprintf(msg, "Speed: %.2f RPS\r\n", speed_sec);
      HAL_UART_Transmit(&huart2, (uint8_t*) msg, strlen(msg), HAL_MAX_DELAY);

      sprintf(msg, "Speed: %.2f RPM\r\n", speed);
      HAL_UART_Transmit(&huart2, (uint8_t*) msg, strlen(msg), HAL_MAX_DELAY);
